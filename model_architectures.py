...
def get_model(model_architecture):
    ...
    model_switcher= {
        ...
        'emb_fnn_flat_dense_linear': emb_fnn_flat_dense_linear(),
        'emb_rnn_dense_linear': emb_rnn_dense_linear(),
        'emb_cnn_dense_linear': emb_cnn_dense_linear(),
...
class nn():
    ...
    def dense_layers(self,final=True):
        ...
        if final:
            ...
            drop[0]=tf.keras.layers.Dropout(rate=dense_dropout)(self.flat_seq_before_dense)
        ...
        if final:
            ...
            return tf.keras.layers.Dense(nodes,activation='relu',name='seq_embedding')(drop[-1])
...
class emb_nn(nn):
    ...
    def input_to_AA_emb(self):
        ...
        xa_len=int(self.xa_len)
        self.input_seq=self.inputs[:,:xa_len]
    ...
    def recombine_cat_var(self):
        xa_len=int(self.xa_len)
        self.input_cat_var=self.inputs[:,xa_len:]
...
class emb_fnn_flat_dense_linear(emb_fnn_flat):
    def __init__(self):
        super().__init__()
    ...
    def set_model(self,space,**kwargs):
        self.set_init_model(space,**kwargs)
        self.input_to_AA_emb()
        self.flat_seq_before_dense=tf.keras.layers.Flatten()(self.AA_embed)
        self.flat_seq=self.dense_layers(final=False)
        self.recombine_cat_var()
        self.outputs=tf.keras.layers.Dense(1,activation=self.lin_or_sig)(self.recombined)
        self.set_end_model()  
...
class emb_rnn_dense_linear(emb_rnn):
    def __init__(self):
        super().__init__()
    ...
    def set_model(self,space,**kwargs):
        ...
        self.set_init_model(space,**kwargs)
        self.input_to_AA_emb()
        self.flat_seq_before_dense=tf.keras.layers.Bidirectional(tf.keras.layers.GRU(units=units,recurrent_dropout=recurrent_dropout,dropout=input_dropout))(self.AA_embed)
        self.flat_seq=self.dense_layers(final=False)
        self.recombine_cat_var()
        self.outputs=tf.keras.layers.Dense(1,activation=self.lin_or_sig)(self.recombined)
        self.set_end_model()  
...
class emb_cnn_dense_linear(emb_cnn):
    def __init__(self):
        super().__init__()
    ...
    def set_model(self,space,**kwargs):
        ...
        self.input_drop=tf.keras.layers.Dropout(rate=input_drop)(self.AA_embed)
        self.cov=tf.keras.layers.Conv1D(filters=filters,kernel_size=kernel_size,activation='relu')(self.input_drop)
        self.flat_seq_before_dense=tf.keras.layers.GlobalMaxPool1D()(self.cov)
        self.flat_seq=self.dense_layers(final=False)
        self.recombine_cat_var()
        self.outputs=tf.keras.layers.Dense(1,activation=self.lin_or_sig)(self.recombined)
        self.set_end_model()  
